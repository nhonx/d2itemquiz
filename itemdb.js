var ITEMS = {
    "belt_of_strength": {
        "index": 1,
        "name": "Belt of Strength"
    },
    "blades_of_attack": {
        "index": 2,
        "name": "Blades of Attack"
    },
    "blade_of_alacrity": {
        "index": 3,
        "name": "Blade of Alacrity"
    },
    "blink_dagger": {
        "index": 4,
        "name": "Blink Dagger"
    },
    "boots_of_speed": {
        "index": 5,
        "name": "Boots of Speed"
    },
    "band_of_elvenskin": {
        "index": 6,
        "name": "Band of Elvenskin"
    },
    "bottle": {
        "index": 7,
        "name": "Bottle"
    },
    "iron_branch": {
        "index": 8,
        "name": "Iron Branch"
    },
    "broadsword": {
        "index": 9,
        "name": "Broadsword"
    },
    "chainmail": {
        "index": 10,
        "name": "Chainmail"
    },
    "circlet": {
        "index": 11,
        "name": "Circlet"
    },
    "clarity": {
        "index": 12,
        "name": "Clarity"
    },
    "claymore": {
        "index": 13,
        "name": "Claymore"
    },
    "cloak": {
        "index": 14,
        "name": "Cloak"
    },
    "animal_courier": {
        "index": 15,
        "name": "Animal Courier"
    },
    "demon_edge": {
        "index": 17,
        "name": "Demon Edge"
    },
    "dust_of_appearance": {
        "index": 18,
        "name": "Dust of Appearance"
    },
    "eaglesong": {
        "index": 19,
        "name": "Eaglesong"
    },
    "energy_booster": {
        "index": 20,
        "name": "Energy Booster"
    },
    "healing_salve": {
        "index": 21,
        "name": "Healing Salve"
    },
    "gauntlets_of_strength": {
        "index": 22,
        "name": "Gauntlets of Strength"
    },
    "gem_of_true_sight": {
        "index": 23,
        "name": "Gem of True Sight"
    },
    "ghost_scepter": {
        "index": 24,
        "name": "Ghost Scepter"
    },
    "gloves_of_haste": {
        "index": 25,
        "name": "Gloves of Haste"
    },
    "helm_of_iron_will": {
        "index": 26,
        "name": "Helm of Iron Will"
    },
    "hyperstone": {
        "index": 27,
        "name": "Hyperstone"
    },
    "javelin": {
        "index": 28,
        "name": "Javelin"
    },
    "morbid_mask": {
        "index": 29,
        "name": "Morbid Mask"
    },
    "magic_stick": {
        "index": 30,
        "name": "Magic Stick"
    },
    "mantle_of_intelligence": {
        "index": 31,
        "name": "Mantle of Intelligence"
    },
    "mithril_hammer": {
        "index": 32,
        "name": "Mithril Hammer"
    },
    "mystic_staff": {
        "index": 33,
        "name": "Mystic Staff"
    },
    "ogre_club": {
        "index": 34,
        "name": "Ogre Club"
    },
    "platemail": {
        "index": 35,
        "name": "Platemail"
    },
    "point_booster": {
        "index": 36,
        "name": "Point Booster"
    },
    "quarterstaff": {
        "index": 37,
        "name": "Quarterstaff"
    },
    "quelling_blade": {
        "index": 38,
        "name": "Quelling Blade"
    },
    "reaver": {
        "index": 39,
        "name": "Reaver"
    },
    "sacred_relic": {
        "index": 40,
        "name": "Sacred Relic"
    },
    "ring_of_health": {
        "index": 41,
        "name": "Ring of Health"
    },
    "ring_of_protection": {
        "index": 42,
        "name": "Ring of Protection"
    },
    "ring_of_regen": {
        "index": 43,
        "name": "Ring of Regen"
    },
    "robe_of_the_magi": {
        "index": 44,
        "name": "Robe of the Magi"
    },
    "slippers_of_agility": {
        "index": 45,
        "name": "Slippers of Agility"
    },
    "sages_mask": {
        "index": 46,
        "name": "Sage's Mask"
    },
    "staff_of_wizardry": {
        "index": 47,
        "name": "Staff of Wizardry"
    },
    "talisman_of_evasion": {
        "index": 49,
        "name": "Talisman of Evasion"
    },
    "tango": {
        "index": 50,
        "name": "Tango"
    },
    "ultimate_orb": {
        "index": 51,
        "name": "Ultimate Orb"
    },
    "vitality_booster": {
        "index": 52,
        "name": "Vitality Booster"
    },
    "void_stone": {
        "index": 53,
        "name": "Void Stone"
    },
    "observer_ward": {
        "index": 54,
        "name": "Observer Ward"
    },
    "sentry_ward": {
        "index": 55,
        "name": "Sentry Ward"
    },
    "recipe": {
        "index": 56,
        "name": "Recipe"
    },
    "orb_of_venom": {
        "index": 57,
        "name": "Orb of Venom"
    },
    "smoke_of_deceit": {
        "index": 58,
        "name": "Smoke of Deceit"
    },
    "shadow_amulet": {
        "index": 59,
        "name": "Shadow Amulet"
    },
    "soul_booster": {
        "index": 60,
        "name": "Soul Booster"
    },
    "abyssal_blade": {
        "index": 61,
        "name": "Abyssal Blade"
    },
    "armlet_of_mordiggian": {
        "index": 62,
        "name": "Armlet of Mordiggian"
    },
    "skull_basher": {
        "index": 63,
        "name": "Skull Basher"
    },
    "battle_fury": {
        "index": 64,
        "name": "Battle Fury"
    },
    "black_king_bar": {
        "index": 65,
        "name": "Black King Bar"
    },
    "blade_mail": {
        "index": 66,
        "name": "Blade Mail"
    },
    "bloodstone": {
        "index": 67,
        "name": "Bloodstone"
    },
    "bracer": {
        "index": 68,
        "name": "Bracer"
    },
    "buckler": {
        "index": 69,
        "name": "Buckler"
    },
    "butterfly": {
        "index": 70,
        "name": "Butterfly"
    },
    "euls_scepter_of_divinity": {
        "index": 71,
        "name": "Eul's Scepter of Divinity"
    },
    "dagon": {
        "index": 72,
        "name": "Dagon"
    },
    "desolator": {
        "index": 73,
        "name": "Desolator"
    },
    "diffusal_blade": {
        "index": 74,
        "name": "Diffusal Blade"
    },
    "drum_of_endurance": {
        "index": 75,
        "name": "Drum of Endurance"
    },
    "medallion_of_courage": {
        "index": 76,
        "name": "Medallion of Courage"
    },
    "veil_of_discord": {
        "index": 77,
        "name": "Veil of Discord"
    },
    "aghanims_scepter": {
        "index": 79,
        "name": "Aghanim's Scepter"
    },
    "urn_of_shadows": {
        "index": 80,
        "name": "Urn of Shadows"
    },
    "vanguard": {
        "index": 81,
        "name": "Vanguard"
    },
    "vladmirs_offering": {
        "index": 82,
        "name": "Vladmir's Offering"
    },
    "wraith_band": {
        "index": 83,
        "name": "Wraith Band"
    },
    "yasha": {
        "index": 84,
        "name": "Yasha"
    },
    "arcane_boots": {
        "index": 85,
        "name": "Arcane Boots"
    },
    "rod_of_atos": {
        "index": 86,
        "name": "Rod of Atos"
    },
    "sange": {
        "index": 87,
        "name": "Sange"
    },
    "heavens_halberd": {
        "index": 88,
        "name": "Heaven's Halberd"
    },
    "sange_and_yasha": {
        "index": 89,
        "name": "Sange and Yasha"
    },
    "satanic": {
        "index": 90,
        "name": "Satanic"
    },
    "scythe_of_vyse": {
        "index": 91,
        "name": "Scythe of Vyse"
    },
    "shivas_guard": {
        "index": 92,
        "name": "Shiva's Guard"
    },
    "eye_of_skadi": {
        "index": 93,
        "name": "Eye of Skadi"
    },
    "soul_ring": {
        "index": 94,
        "name": "Soul Ring"
    },
    "tranquil_boots": {
        "index": 95,
        "name": "Tranquil Boots"
    },
    "boots_of_travel": {
        "index": 96,
        "name": "Boots of Travel"
    },
    "daedalus": {
        "index": 97,
        "name": "Daedalus"
    },
    "hand_of_midas": {
        "index": 98,
        "name": "Hand of Midas"
    },
    "headdress": {
        "index": 99,
        "name": "Headdress"
    },
    "helm_of_the_dominator": {
        "index": 100,
        "name": "Helm of the Dominator"
    },
    "hood_of_defiance": {
        "index": 101,
        "name": "Hood of Defiance"
    },
    "shadow_blade": {
        "index": 102,
        "name": "Shadow Blade"
    },
    "crystalys": {
        "index": 103,
        "name": "Crystalys"
    },
    "linkens_sphere": {
        "index": 104,
        "name": "Linken's Sphere"
    },
    "maelstrom": {
        "index": 105,
        "name": "Maelstrom"
    },
    "magic_wand": {
        "index": 106,
        "name": "Magic Wand"
    },
    "manta_style": {
        "index": 107,
        "name": "Manta Style"
    },
    "mask_of_madness": {
        "index": 108,
        "name": "Mask of Madness"
    },
    "assault_cuirass": {
        "index": 109,
        "name": "Assault Cuirass"
    },
    "mekansm": {
        "index": 110,
        "name": "Mekansm"
    },
    "mjollnir": {
        "index": 111,
        "name": "Mjollnir"
    },
    "monkey_king_bar": {
        "index": 112,
        "name": "Monkey King Bar"
    },
    "null_talisman": {
        "index": 114,
        "name": "Null Talisman"
    },
    "oblivion_staff": {
        "index": 115,
        "name": "Oblivion Staff"
    },
    "ethereal_blade": {
        "index": 116,
        "name": "Ethereal Blade"
    },
    "force_staff": {
        "index": 117,
        "name": "Force Staff"
    },
    "heart_of_tarrasque": {
        "index": 118,
        "name": "Heart of Tarrasque"
    },
    "refresher_orb": {
        "index": 119,
        "name": "Refresher Orb"
    },
    "ring_of_basilius": {
        "index": 120,
        "name": "Ring of Basilius"
    },
    "orchid_malevolence": {
        "index": 121,
        "name": "Orchid Malevolence"
    },
    "perseverance": {
        "index": 122,
        "name": "Perseverance"
    },
    "phase_boots": {
        "index": 123,
        "name": "Phase Boots"
    },
    "pipe_of_insight": {
        "index": 124,
        "name": "Pipe of Insight"
    },
    "power_treads": {
        "index": 126,
        "name": "Power Treads"
    },
    "radiance": {
        "index": 127,
        "name": "Radiance"
    },
    "divine_rapier": {
        "index": 128,
        "name": "Divine Rapier"
    },
    "silver_edge": {
        "index": 129,
        "name": "Silver Edge"
    },
    "solar_crest": {
        "index": 130,
        "name": "Solar Crest"
    },
    "guardian_greaves": {
        "index": 131,
        "name": "Guardian Greaves"
    },
    "lotus_orb": {
        "index": 132,
        "name": "Lotus Orb"
    },
    "moon_shard": {
        "index": 133,
        "name": "Moon Shard"
    },
    "octarine_core": {
        "index": 134,
        "name": "Octarine Core"
    },
    "glimmer_cape": {
        "index": 135,
        "name": "Glimmer Cape"
    },
    "aether_lens": {
        "index": 136,
        "name": "Aether Lens"
    },
    "bloodthorn": {
        "index": 137,
        "name": "Bloodthorn"
    },
    "crimson_guard": {
        "index": 138,
        "name": "Crimson Guard"
    },
    "echo_sabre": {
        "index": 139,
        "name": "Echo Sabre"
    },
    "hurricane_pike": {
        "index": 140,
        "name": "Hurricane Pike"
    },
    "blight_stone": {
        "index": 142,
        "name": "Blight Stone"
    },
    "dragon_lance": {
        "index": 143,
        "name": "Dragon Lance"
    },
    "wind_lace": {
        "index": 144,
        "name": "Wind Lace"
    },
    "infused_raindrop": {
        "index": 146,
        "name": "Infused Raindrop"
    },
    "enchanted_mango": {
        "index": 147,
        "name": "Enchanted Mango"
    },
    "meteor_hammer": {
        "index": 148,
        "name": "Meteor Hammer"
    },
    "aeon_disk": {
        "index": 149,
        "name": "Aeon Disk"
    },
    "spirit_vessel": {
        "index": 150,
        "name": "Spirit Vessel"
    },
    "nullifier": {
        "index": 151,
        "name": "Nullifier"
    },
    "kaya": {
        "index": 152,
        "name": "Kaya"
    },
    //===========================================================================
    "voodoo_mask": {
        "index": 152,
        "name": "Voodoo Mask"
    },
    "falcon_blade": {
        "index": 152,
        "name": "Falcon Blade"
    },
    "helm_of_the_overlord": {
        "index": 152,
        "name": "Helm of the Overlord"
    },
    "holy_locket": {
        "index": 152,
        "name": "Holy Locket"
    },
    "witch_blade": {
        "index": 152,
        "name": "Witch Blade"
    },
    "gleipnir": {
        "index": 152,
        "name": "Gleipnir"
    },
    "wind_waker": {
        "index": 152,
        "name": "Wind Waker"
    },
    "eternal_shroud": {
        "index": 152,
        "name": "Eternal Shroud"
    },
    "mage_slayer": {
        "index": 152,
        "name": "Mage Slayer"
    },
    "kaya_and_sange": {
        "index": 152,
        "name": "Kaya And Sange"
    },
    "yasha_and_kaya": {
        "index": 152,
        "name": "Yasha and Kaya"
    },
    "arcane_blink": {
        "index": 152,
        "name": "arcane Blink"
    },
    "swift_blink": {
        "index": 152,
        "name": "Swift Blink"
    },
    "overwhelming_blink": {
        "index": 152,
        "name": "Overwhelming Blink"
    },
    "crown": {
        "index": 152,
        "name": "Crown"
    },
    "blitz_knuckles": {
        "index": 152,
        "name": "Blitz Knuckles"
    },
    "fluffy_hat": {
        "index": 152,
        "name": "Fluffy Hat"
    },
    "orb_of_corrosion": {
        "index": 152,
        "name": "Orb of Corrosion"
    }
}

var ITEM_REL = {
    "soul_booster": [
        "energy_booster",
        "point_booster",
        "vitality_booster"
    ],
    "abyssal_blade": [
        "skull_basher",
        "vanguard",
        "recipe"
    ],
    "armlet_of_mordiggian": [
        "recipe",
        "helm_of_iron_will",
        "gloves_of_haste",
        "blades_of_attack"
    ],
    "skull_basher": [
        "belt_of_strength",
        "javelin",
        "recipe"
    ],
    "battle_fury": [
        "perseverance",
        "broadsword",
        "claymore",
        "quelling_blade"
    ],
    "black_king_bar": [
        "mithril_hammer",
        "ogre_club",
        "recipe"
    ],
    "blade_mail": [
        "broadsword",
        "chainmail",
        "robe_of_the_magi"
    ],
    "bloodstone": [
        "soul_booster",
        "kaya",
        "voodoo_mask"
    ],
    "bracer": [
        "gauntlets_of_strength",
        "recipe",
        "circlet"
    ],
    "buckler": [
        "ring_of_protection",
        "recipe"
    ],
    "butterfly": [
        "quarterstaff",
        "eaglesong",
        "talisman_of_evasion"
    ],
    "euls_scepter_of_divinity": [
        "staff_of_wizardry",
        "wind_lace",
        "void_stone",
        "recipe"
    ],
    "dagon": [
        "staff_of_wizardry",
        "crown",
        "recipe"
    ],
    "desolator": [
        "mithril_hammer",
        "mithril_hammer",
        "blight_stone"
    ],
    "diffusal_blade": [
        "blade_of_alacrity",
        "blade_of_alacrity",
        "recipe",
        "robe_of_the_magi"
    ],
    "drum_of_endurance": [
        "robe_of_the_magi",
        "belt_of_strength",
        "wind_lace",
        "recipe"
    ],
    "medallion_of_courage": [
        "chainmail",
        "sages_mask",
        "blight_stone"
    ],
    "veil_of_discord": [
        "ring_of_basilius",
        "crown",
        "recipe"
    ],
    "aghanims_scepter": [
        "blade_of_alacrity",
        "ogre_club",
        "staff_of_wizardry",
        "point_booster"
    ],
    "urn_of_shadows": [
        "sages_mask",
        "circlet",
        "ring_of_protection",
        "recipe"
    ],
    "vanguard": [
        "ring_of_health",
        "vitality_booster"
    ],
    "vladmirs_offering": [
        "ring_of_basilius",
        "buckler",
        "morbid_mask",
        "blades_of_attack",
        "recipe"
    ],
    "wraith_band": [
        "circlet",
        "recipe",
        "slippers_of_agility"
    ],
    "yasha": [
        "blade_of_alacrity",
        "band_of_elvenskin",
        "recipe"
    ],
    "arcane_boots": [
        "boots_of_speed",
        "energy_booster"
    ],
    "rod_of_atos": [
        "staff_of_wizardry",
        "crown",
        "crown",
        "recipe"
    ],
    "sange": [
        "belt_of_strength",
        "ogre_club",
        "recipe"
    ],
    "heavens_halberd": [
        "sange",
        "talisman_of_evasion",
        "recipe"
    ],
    "sange_and_yasha": [
        "yasha",
        "sange"
    ],
    "yasha_and_kaya": [
        "yasha",
        "kaya"
    ],
    "kaya_and_sange": [
        "kaya",
        "sange"
    ],
    "satanic": [
        "morbid_mask",
        "reaver",
        "claymore"
    ],
    "scythe_of_vyse": [
        "mystic_staff",
        "void_stone",
        "ultimate_orb"
    ],
    "shivas_guard": [
        "mystic_staff",
        "platemail",
        "recipe"
    ],
    "eye_of_skadi": [
        "ultimate_orb",
        "ultimate_orb",
        "point_booster"
    ],
    "soul_ring": [
        "ring_of_protection",
        "gauntlets_of_strength",
        "gauntlets_of_strength",
        "recipe"
    ],
    "tranquil_boots": [
        "boots_of_speed",
        "ring_of_regen",
        "wind_lace"
    ],
    "boots_of_travel": [
        "boots_of_speed",
        "recipe"
    ],
    "daedalus": [
        "crystalys",
        "recipe",
        "demon_edge"
    ],
    "hand_of_midas": [
        "gloves_of_haste",
        "recipe"
    ],
    "headdress": [
        "ring_of_regen",
        "recipe"
    ],
    "helm_of_the_dominator": [
        "helm_of_iron_will",
        "crown",
        "recipe"
    ],
    "hood_of_defiance": [
        "ring_of_health",
        "ring_of_regen",
        "cloak"
    ],
    "shadow_blade": [
        "shadow_amulet",
        "broadsword",
        "blitz_knuckles"
    ],
    "crystalys": [
        "recipe",
        "blades_of_attack",
        "broadsword"
    ],
    "linkens_sphere": [
        "perseverance",
        "recipe",
        "ultimate_orb"
    ],
    "maelstrom": [
        "mithril_hammer",
        "javelin"
    ],
    "mage_slayer": [
        "cloak",
        "oblivion_staff",
        "recipe",
    ],
    "magic_wand": [
        "recipe",
        "iron_branch",
        "iron_branch",
        "magic_stick"
    ],
    "manta_style": [
        "yasha",
        "ultimate_orb",
        "recipe"
    ],
    "mask_of_madness": [
        "oblivion_staff",
        "morbid_mask"
    ],
    "assault_cuirass": [
        "hyperstone",
        "platemail",
        "recipe",
        "chainmail"
    ],
    "mekansm": [
        "recipe",
        "chainmail",
        "headdress"
    ],
    "mjollnir": [
        "maelstrom",
        "hyperstone",
        "recipe"
    ],
    "gleipnir": [
        "maelstrom",
        "rod_of_atos",
        "recipe"
    ],
    "wind_waker": [
        "euls_scepter_of_divinity",
        "mystic_staff",
        "recipe"
    ],
    "monkey_king_bar": [
        "demon_edge",
        "javelin",
        "blitz_knuckles",
        "recipe"
    ],
    "null_talisman": [
        "circlet",
        "recipe",
        "mantle_of_intelligence"
    ],
    "oblivion_staff": [
        "quarterstaff",
        "sages_mask",
        "robe_of_the_magi"
    ],
    "ethereal_blade": [
        "ghost_scepter",
        "eaglesong"
    ],
    "force_staff": [
        "staff_of_wizardry",
        "fluffy_hat",
        "recipe"
    ],
    "heart_of_tarrasque": [
        "reaver",
        "vitality_booster",
        "recipe"
    ],
    "refresher_orb": [
        "perseverance",
        "perseverance",
        "recipe"
    ],
    "ring_of_basilius": [
        "recipe",
        "sages_mask"
    ],
    "orchid_malevolence": [
        "oblivion_staff",
        "oblivion_staff",
        "recipe"
    ],
    "perseverance": [
        "void_stone",
        "ring_of_health"
    ],
    "phase_boots": [
        "boots_of_speed",
        "chainmail",
        "blades_of_attack"
    ],
    "pipe_of_insight": [
        "hood_of_defiance",
        "headdress",
        "recipe"
    ],
    "power_treads": [
        "belt_of_strength",
        "boots_of_speed",
        "gloves_of_haste"
    ],
    "radiance": [
        "sacred_relic",
        "recipe"
    ],
    "divine_rapier": [
        "sacred_relic",
        "demon_edge"
    ],
    "silver_edge": [
        "shadow_blade",
        "crystalys",
        "recipe"
    ],
    "solar_crest": [
        "crown",
        "wind_lace",
        "recipe",
        "medallion_of_courage"
    ],
    "guardian_greaves": [
        "arcane_boots",
        "mekansm",
        "buckler",
        "recipe"
    ],
    "lotus_orb": [
        "platemail",
        "perseverance",
        "energy_booster"
    ],
    "moon_shard": [
        "hyperstone",
        "hyperstone",
    ],
    "octarine_core": [
        "aether_lens",
        "soul_booster"
    ],
    "glimmer_cape": [
        "shadow_amulet",
        "cloak",
        "recipe"
    ],
    "aether_lens": [
        "energy_booster",
        "void_stone",
        "recipe"
    ],
    "bloodthorn": [
        "orchid_malevolence",
        "hyperstone",
        "recipe"
    ],
    "crimson_guard": [
        "vanguard",
        "helm_of_iron_will",
        "recipe"
    ],
    "echo_sabre": [
        "ogre_club",
        "oblivion_staff"
    ],
    "hurricane_pike": [
        "force_staff",
        "dragon_lance",
        "recipe"
    ],
    "dragon_lance": [
        "ogre_club",
        "band_of_elvenskin",
        "band_of_elvenskin"
    ],
    "falcon_blade": [
        "fluffy_hat",
        "blades_of_attack",
        "sages_mask",
        "recipe"
    ],
    "meteor_hammer": [
        "perseverance",
        "crown",
        "recipe",
    ],
    "aeon_disk": [
        "energy_booster",
        "vitality_booster",
        "recipe"
    ],
    "eternal_shroud": [
        "voodoo_mask",
        "hood_of_defiance",
        "recipe"
    ],
    "spirit_vessel": [
        "urn_of_shadows",
        "vitality_booster",
        "recipe"
    ],
    "holy_locket": [
        "headdress",
        "fluffy_hat",
        "magic_wand",
        "energy_booster",
        "recipe"
    ],
    "nullifier": [
        "sacred_relic",
        "helm_of_iron_will"
    ],
    "kaya": [
        "recipe",
        "robe_of_the_magi",
        "staff_of_wizardry"
    ],
    "helm_of_the_overlord": [
        "recipe",
        "helm_of_the_dominator",
        "vladmirs_offering"
    ],
    "witch_blade": [
        "chainmail",
        "recipe",
        "blitz_knuckles",
        "robe_of_the_magi"
    ],
    "orb_of_corrosion": [
        "orb_of_venom",
        "blight_stone",
        "fluffy_hat",
        "recipe"
    ],
    "swift_blink": [
        "blink_dagger",
        "recipe",
        "eaglesong",
    ],
    "arcane_blink": [
        "blink_dagger",
        "recipe",
        "mystic_staff"
    ],
    "overwhelming_blink": [
        "blink_dagger",
        "recipe",
        "reaver"
    ]
}
var ITEM_COUNT = Object.keys(ITEMS).length - 1;
var ITEMS_IN_ELEMENT = [];
var HISTORY_ITEM = [];
var EXCLUDE_IDS = [];
function verify_db() {
    const all_item = Object.keys(ITEMS);
    let all_combined_items = [];
    let all_toCombined_items = [];
    Object.keys(ITEM_REL).forEach(x => {
        const toAdd = [x, ...ITEM_REL[x]].filter(k => !all_combined_items.includes(k));
        all_toCombined_items =[...all_toCombined_items,...ITEM_REL[x].filter(k => !all_toCombined_items.includes(k))]
        all_combined_items = [...all_combined_items, ...toAdd]
    });
    console.log("All items", ITEM_COUNT);
    console.log("All combinable items", Object.keys(ITEM_REL).length);
    console.log("All items to combine", all_toCombined_items.length);
    ITEMS_IN_ELEMENT = all_toCombined_items;
    if (all_combined_items.filter(x => !all_item.includes(x)).length > 0) {
        console.log("Untracked items detected.", all_combined_items.filter(x => !all_item.includes(x)))
    }
}
verify_db();